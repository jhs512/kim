...
- "비서 {코드}"로 어느 비서(kim1 등)를 쓸지 고른다. **한 프로젝트 = 비서 1명**, 여러 명이면 프로젝트를 여러 개.
...
# 인스턴스 루트에 kim.config.json → {"store":"kim1"}   (kim.config.json.example 참고)
...
- "비서 kim1, 지금 비서코드 뭐야?" → "kim1 입니다." `[선택]`
...
### 노드 시트 이름 (폰이 읽는 파생 뷰)
`{비서코드}_{no}_{namespace}_{doctype}_{visibility}_{title}` (예: `kim1_1_personal_concepts_public_복리`).
